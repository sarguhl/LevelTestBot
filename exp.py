...
class Exp(Cog):
	...
	async def check_lvl_rewards(self, message, lvl):
		if lvl >= 50: # Red
			if (new_role := message.guild.get_role(823688657054859304)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(823688626176131083))

		elif 40 <= lvl < 50: # Yellow
			if (new_role := message.guild.get_role(823688626176131083)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(823688603829534772))

		elif 30 <= lvl < 40: # Green
			if (new_role := message.guild.get_role(823688603829534772)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(823688574880841759))

		elif 20 <= lvl < 30: # Blue
			if (new_role := message.guild.get_role(823688574880841759)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(823688544375275530))

		elif 10 <= lvl < 20: # Purple
			if (new_role := message.guild.get_role(823688544375275530)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(823688504869388349))

		elif 5 <= lvl < 9: # Black
			if (new_role := message.guild.get_role(823688504869388349)) not in message.author.roles:
				await message.author.add_roles(new_role)
	...
	async def on_ready(self):
		if not self.bot.ready:
			self.levelup_channel = self.bot.get_channel(823688417002389504)
