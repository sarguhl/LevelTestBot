...
class Bot(BotBase):
    ...
    def update_db(self):
        ...
        print("All new guilds added")

        db.multiexec("INSERT OR IGNORE INTO exp (UserID) VALUES (?)",
                    ((member.id,) for member in self.guild.members if not member.bot))
        print("All new users added")
    ...
    def run(self, version):
        ...
        with open("./data/rest/token.txt", "r", encoding="utf-8") as tf:
            self.TOKEN = tf.read()
    ...
    async def on_ready(self):
        if not self.ready:
            self.guild = self.get_guild(823688367044296724)
            self.stdout = self.get_channel(823690016185516084)
